...
#include <stdint.h>
#include "C:\Users\yiyua\Documents\Arduino\libraries\micro-ecc\uECC.h"
#include "keccak.h"
#include "tx.h"
#include "config.h"
...
LiquidCrystal lcd(4, 16, 17, 5, 21, 33); //for ESP32
...
void setup() {
  ...
  pinMode(3, OUTPUT);
  digitalWrite(3,LOW); 
  pinMode(TASTER, INPUT_PULLUP); //D2 is for taster. when the taster is pressed, D2 is LOW
  lcd.begin(16,2);
  lcd.setCursor(0,0);
  lcd.print("WLAN???");
...
void loop() { 
  // Only run loop if maximum ammount of desired transactions was not yet reached.
  
  String sContractSignal = "";
  int intContractSignal = 0;
  int loop_number = 0; //loop number for type1
  
  int TasterVal = digitalRead(TASTER);
  Serial.println("end5 ");
//  int distance;
  ////Serial.print("Taster value: ");
  ////Serial.println(TasterVal);
  // if button is pressed, then change the type.
  int still_loop = 0;
   ...
   while (variate == false){
    ...
    String _Transaction="";       
      ...
      if(transaction.nonce != "invalid")
      {
        ...
        // if the nonce is too small, get a new nonce
